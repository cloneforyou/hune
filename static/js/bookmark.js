  ...
  function parseDOM (obj) {
    ...
    if (count === 0) {
      ...
      obj.items.forEach(item => {
        html += `
        <div class="link" >
         <a class="link-title" href="${item.link}" target="_blank" rel="nofollow noopener">${item.title}
          <div class="meta">
            <a href="https://${item.site}" target="_blank" rel="nofollow noopener">${item.site}</a> | <a href="#" class="js-b-dropdown-remove-bookmark" data-id="${item.link}">remove</a>
          </div>
        </div>
        `
      });
